...
public class MemberMenu {
	...
	public void mainMenu() {
		...
		while(true) {
			...
			case "6" : 
				...
				break;
			case "7" :
				list = memberController.selectDeleteAll();
				printDeleteMemberList(list);
				break;
			case "0" : return;
			default :
    ...
    public void printDeleteMemberList(List<Member> list) {
        if(list == null || list.isEmpty()) {
            System.out.println("> 조회된 행이 없습니다.");
        }
        else {
            ...
            System.out.printf("%15s%15s%15s%15s%15s%15s%15s%15s%n", 
                                "id", "name", "gender", "birthday", "email", "address", "regDate", "del_date");
            System.out.println("-----------------------------------------------------");
            for(Member member : list) {
                System.out.printf("%15s%15s%15s%15s%15s%15s%15s%15s%n", 
                        member.getId(), 
